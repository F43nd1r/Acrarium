 ...
 * @since 17.09.18
 */
class ReportList(app: App, private val dataProvider: QueryDslDataProvider<Report>, avatarService: AvatarService, reportDeleter: Consumer<Report>) : Composite<AcrariumGrid<Report>>() {
    init {
        content.run {
            ...
            val dateColumn = addColumn(TimeSpanRenderer(ValueProvider { obj: Report -> obj.date })).setSortable(QReport.report.date).setCaption(Messages.DATE)

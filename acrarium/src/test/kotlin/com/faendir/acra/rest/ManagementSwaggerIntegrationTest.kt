...
class ManagementSwaggerIntegrationTest(
    ...
    @Test
    fun `should require admin authentication for management swagger ui`() {
        val result = restTemplate.getForEntity("http://localhost:$managementPort/actuator/swagger-ui", String::class.java)

        expectThat(result.statusCode).isEqualTo(HttpStatus.UNAUTHORIZED)
    }

    @Test
    fun `should serve management swagger ui and grouped openapi documents to admins`() {
        val swaggerUi = restTemplate.withBasicAuth(username, password).getForEntity("http://localhost:$managementPort/actuator/swagger-ui/index.html", String::class.java)
